...
import { BYTE_LENGTH, DISPLAY_HEIGHT, DISPLAY_WIDTH, FLAG_REGISTER, FONT_DATA, HIGH_RES_DISPLAY_HEIGHT, HIGH_RES_DISPLAY_WIDTH, LARGE_FONT_DATA_START, MEMORY_SIZE, } from './constants.js';
import { createDisplay } from './display.js';
import { doubleByte, getHigherNibble, getLowerNibble, getNthBit, joinNibbles, matchInstruction, mod, } from './utils.js';
export class SuperChip48Vm extends Chip8Vm {
    ...
    scrollRight() {
        for (let y = 0; y < this.display.height; y++) {
            for (let x = this.display.width / BYTE_LENGTH - 1; x > 0; x--) {
                ...
                const previousLowerNibble = getLowerNibble(this.display.display.array[index - 1]);
            ...
            this.display.display.array[index] = getHigherNibble(this.display.display.array[index]);
        }
    }
    scrollLeft() {
        for (let y = 0; y < this.display.height; y++) {
            for (let x = 0; x < this.display.width / BYTE_LENGTH - 1; x++) {
                const index = (y * this.display.width) / BYTE_LENGTH + x;
                const nextHigherNibble = getHigherNibble(this.display.display.array[index + 1]);
    ...
    drawHighResolution(instruction) {
        ...
        for (let dy = 0; dy < height; dy++) {
            for (let dx = 0; dx < BYTE_LENGTH * 2; dx++) {
                const spriteByte = this.memory[this.registerI + dy * 2 + Math.floor(dx / BYTE_LENGTH)];
                const x = originX + dx;
                const y = originY + dy;
    ...
    highResolution() {
        this.display = createDisplay(HIGH_RES_DISPLAY_WIDTH, HIGH_RES_DISPLAY_HEIGHT);
    }
    or(instruction) {
    ...
    jumpRelativeAddress(instruction) {
        const register = instruction[1];
        const address = joinNibbles(instruction.slice(1)) + this.registers[register];
